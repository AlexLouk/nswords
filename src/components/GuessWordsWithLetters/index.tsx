...
export const GuessWordsWithLetters = () => {
    ...
    const needHelp = (firstAvailable?: boolean) => {
        setHelpDefinition(
            (firstAvailable
                ? wordsRemaining.find((w) => !!w.definition)
                : wordsRemaining[
                      Math.round(Math.random() * wordsRemaining.length)
                  ]) ?? null
        )
    }
    ...
    return (
        <div>
            <h2 className="mt-1 text-bluegray-200 text-center">
                {t('guessAllWords')}
                <i
                    className="pi pi-cog ml-4 cursor-pointer hover:text-primary"
            ...
            <div className="flex flex-wrap gap-5 row-gap-4 justify-content-center relative">
                {!!wordsToGuess.length && (
                    <div
                        className="text-right absolute z-5"
                        style={{ top: '-10px', right: '-15px' }}
                    >
                        <i
                            className="pi pi-info-circle text-lg"
                ...
                {wordsToGuess.map((w, i) => {
                    ...
                    return (
                        <div
                            key={i}
                            className={`lettersContainer ${found && 'box-ok'}`}
                        >
                            {w.key.split('').map((l, k) => (
                                <div key={k} className="letterBox">
                                    {(found || !!lettersHelp?.[i]?.[k]) && l}
                                </div>
                            ))}
                        </div>
                    )
            ...
            <div className="h-2rem mt-5 mb-2 text-center text-primary">
                ...
                {!!selectedWord && (
                    <span style={{ letterSpacing: '5px' }}>{selectedWord}</span>
                )}
                {!selectedWord &&
                    (trials === 5 ? (
                        <a
                            className="cursor-pointer text-red-500"
                            onClick={() => addLetterHelp()}
                        >
                            {t('needLetter')}
                        </a>
                    ) : trials === 4 ? (
                        ...
                        <span className="text-red-200">{t('almostOrNot')}</span>
                    ) : (
                        trials === 1 && (
                            <span className="text-red-100">{t('failed')}</span>
                        )
                    ))}
            </div>
            <Divider className="mt-1" />
            {!!wordsRemaining.length && (
                <div
                    className={`mt-2 text-center relative ${!buttonsMode && 'h-15rem'}`}
                >
                    {!!wordsToGuess.length && (
                        <div
                            className="text-right absolute top-0 z-5"
                            style={{ right: '-15px' }}
                        >
                            <i
                                className="pi pi-refresh text-lg"
                                onClick={() => reshuffleLetters()}
                            />
                        </div>
                    )}
                    {!forcedButtonsMode && !!wordsToGuess.length && (
                        <div
                            className="text-right absolute z-5"
                            style={{ top: '30px', right: '-15px' }}
                        >
                            <i
                                className="pi pi-table text-lg"
                                onClick={() => setButtonsMode(!buttonsMode)}
                            />
                        </div>
                    )}
                    {!buttonsMode && (
                        <LetterCanvas
                            letters={letters}
                            onWordComplete={(word) => onWordComplete(word)}
                            onLetterSelected={(l) =>
                                setSelectedWord((selectedWord ?? '') + l)
                            }
                        />
                    )}
                    {buttonsMode && (
            ...
            <div className="mt-6 text-center">
                <Button severity="info" onClick={() => reset()}>
            ...
            <Dialog
                header="Definitions"
                visible={helpDefinition !== null}
                onHide={() => {
                    setHelpDefinition(null)
                    setTrials(0)
                }}
                ...
                {helpDefinition !== null && (
                    <>
                        <div className="font-italic text-sm mb-2">
                            {helpDefinition.key.length} {t('letters')}
                        </div>
                        <div
                            className={
